...
% if is_general:
REM delete reg in ghost task for rename, reboot, copy setupEnv.bat, and execute setupEnv.bat
REG DELETE HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v NET_USE /f >> C:\setupenv.log 2>&1
REG DELETE HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v SETUPENV /f >> C:\setupenv.log 2>&1
% endif
...
copy /V /Y /Z "${script_ini_dir_server}\script.ini" .

REM add notification sentense to boot script.
echo set /p GHOST_STATUS=^<ghostStatus.log >> boot.bat
echo "C:\Python26\python.exe" "nicu\notify.py" -t GhostAgent -e GhostClient --status "%%GHOST_STATUS%%" ${machine_name} >> boot.bat

echo Start to run boot.bat
% if is_general:
call boot.bat "${boot_dir_home}"
% else:
reg ADD HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /f /v BOOT_RESTART /t REG_SZ /d "cmd /S /C \"\"${'%workdir:~1,-1%'}\boot.bat\" \"${boot_dir_home}\"\""
shutdown /f /r /t 10
% endif
