...
#
# Additional code Copyright (c) 2009 Adam Vandenberg.
# http://github.com/adamv/p4lib/tree/master
"""
    A Python wrapper 'p4', the Perforce client command-line app.
    ...
    For more information see the doc string on each command. For example:
        help(p4lib.P4.opened)

    Implemented commands:
        add (limited test suite), branch, branches, change, changes (no
        test suite), client, clients, delete, describe (no test suite),
        diff, dirs (no test suite), edit (no test suite), files (no test
        suite), filelog (no test suite), flush, have (no test suite), label,
        labels, opened, print (as print_, no test suite), resolve,
        revert (no test suite), submit, sync, where (no test suite),
        fstat (no test suite)
    ...
    Unimplemented commands:
        admin, counter, counters, depot, depots, fix, fixes,
...
__all__ = ['P4LibError', 'P4', 'makeForm', 'parseForm', 'makeOptv', 'parseOptv']
...
class _Logger:
    ...
    def __init__(self, threshold=None, streamOrFileName=sys.stderr):
        ...
        if isinstance(streamOrFileName, basestring):
            self.stream = open(streamOrFileName, 'w')
            self._opennedStream = True
        else:
            self.stream = streamOrFileName
            self._opennedStream = False
...
def _escapeArg(arg):
    ...
    #XXX This is also not right on Linux, just try putting 'p4' is a dir with spaces.
...
def _joinArgv(argv):
    ...
    for arg in argv:
        ...
        if ' ' in arg or '*' in arg:
            parts.append('"%s"' % _escapeArg(arg))
        else:
            parts.append(_escapeArg(arg))
    return ' '.join(parts)
...
def _specialsLast(a, b, specials):
    """Alphabetical sort with items from `specials` sorted last.
    """
...
def makeOptv(**options):
    ...
    for key, val in options.items():
        if val is None:
            continue
        try:
            optv.extend([param_names[key], val])
        except KeyError:
...
class P4:
    ...
    def opened(self, files=[], allClients=0, change=None, _raw=False,
               **p4options):
    ...
    def where(self, files=None, _raw=False, **p4options):
        """Show how filenames map through the client view.

        `files` is a list of files or file wildcards to check.
        Defaults to the whole client view.
        ...
        When `_raw` is True, returns a dictionary with the unprocessed results of calling p4:
            {'stdout': <stdout>, 'stderr': <stderr>, 'retval': <retval>}
        """
        # Output examples:
        ...
        # The last one is surprising; it comes from using '*' in the client spec.
        if isinstance(files, basestring):
            files = [files]
    ...
    def have(self, files=[], _raw=False, **p4options):
    ...
    def describe(self, change, diffFormat='', shortForm=0, _raw=False,
                 **p4options):
    ...
    def change(self, files=None, description=None, change=None, delete=0,
        ...
        try:
            ...
            if change and files is None and not description:
                ...
                if delete:
                    raise P4LibError("Cannot specify 'delete' with either 'files' or 'description'.")
                if change:
    ...
    def changes(self, files=[], followIntegrations=0, longOutput=0,
                max=None, status=None, _raw=False, **p4options):
    ...
    def sync(self, files=[], force=0, dryrun=0, _raw=False, **p4options):
    ...
    def edit(self, files, change=None, filetype=None, _raw=False, **p4options):
    ...
    def add(self, files, change=None, filetype=None, _raw=False, **p4options):
    ...
    def modify(self, files, change=None, filetype=None, **p4options):
        """Adds or edits files as appropriate. Raw output is not allowed."""
        edit_hits = self.edit(files, change, filetype, False, **p4options)
        to_add = list()
        combined_hits = list()

        for hit in edit_hits:
            if 'not on client' in hit['comment']:
                to_add.append(hit['depotFile'])
            else:
                combined_hits.append(hit)

        add_hits = self.add(files, change, filetype, False, **p4options)

        combined_hits.extend(add_hits)
        return combined_hits

    def files(self, files, _raw=False, **p4options):
    ...
    def dirs(self, dirs, _raw=False, **p4options):
        """List the immediate subfolders of the given folders.
        """
    ...
    def print_(self, files, localFile=None, quiet=False, **p4options):
        ...
        startHitWithNextNode = True
        for node in _unmarshal(o):
            if node['code'] == 'info':
                ...
                hit['rev'] = int(hit['rev'])
                hits.append(hit)
                startHitWithNextNode = False
            elif node['code'] == 'text':
                if startHitWithNextNode:
                    hit = {'text': node['data']}
                    hits.append(hit)
                else:
                    if not 'text' in hits[-1]\
    ...
    def diff(self, files=[], diffFormat='', force=False, satisfying=None,
             text=False, _raw=False, **p4options):
    ...
    def diff2(self, file1, file2, diffFormat='', quiet=False, text=False,
              **p4options):
        ...
        for node in _unmarshal(o):
            if node['code'] == 'info'\
               and node['data'] == '(... files differ ...)':
                ...
                if not 'text' in diff or diff['text'] is None:
                    diff['text'] = node['data']
                else:
                    diff['text'] += node['data']
    ...
    def revert(self, files=[], change=None, unchangedOnly=0, _raw=False,
               **p4options):
    ...
    def submit(self, files=None, description=None, change=None, _raw=False,
               **p4options):
    ...
    def delete(self, files, change=None, _raw=False, **p4options):
    ...
    def client(self, name=None, client=None, delete=False, _raw=False, **p4options):
    ...
    def clients(self, _raw=False, **p4options):
        ...
        for line in output.splitlines(1):
            ...
            if not match:
                raise P4LibError(
                    "Internal error: could not parse 'p4 clients' output line: '%s'" % line)

            client = match.groupdict()
            clients.append(client)
        return clients

    def label(self, name=None, label=None, delete=False, _raw=False, **p4options):
        ...
        try:
            ...
            if action == 'get':
                ...
                if match:
                    ...
                    err = "Internal error: could not parse p4 label output: '%s'" % output
                    raise P4LibError(err)
            else:
                raise P4LibError("Internal error: unexpected action: '%s'" % action)
    ...
    def labels(self, _raw=False, **p4options):
    ...
    def branch(self, name=None, branch=None, delete=False, _raw=False, **p4options):
    ...
    def branches(self, _raw=False, **p4options):
    ...
    def fstat(self, files, _raw=False, **p4options):
        """List files in the depot.
